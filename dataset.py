...
class MyLidcDataset(Dataset):
    def __init__(self, IMAGES_PATHS, MASK_PATHS,CROP_SIZE=512,Albumentation=False):
        """
    ...
    def transform(self, image, mask,index):
        ...
        if self.albumentation:
            ...
            try:
                image = image.reshape(self.crop_size,self.crop_size,1)
            except:
                print(image.shape)
                image = image.reshape(self.crop_size,self.crop_size,1)
            mask = mask.reshape(self.crop_size,self.crop_size,1)
            ...
            mask = augmented['mask']
            mask= mask.reshape([1,self.crop_size,self.crop_size])
        else:
