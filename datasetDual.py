...
class MyLidcDataset(Dataset):
    ...
    def transform(self, image, mask,index):
        ...
        if self.albumentation:
            ...
            bbox = self.transformations(bbox)      
        image,mask,bbox = image.type(torch.FloatTensor), mask.type(torch.FloatTensor), bbox.type(torch.FloatTensor)
        return image,mask, bbox

    def __getitem__(self, index):
        image = np.load(self.image_paths[index])
        mask = np.load(self.mask_paths[index])        
        image,mask, bbox = self.transform(image,mask,index)
        return image,mask, bbox
