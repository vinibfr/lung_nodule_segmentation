...
class SAModule(nn.Module):
    ...
    def forward(self, input_feature, annotator_clicks):
        # Concatenate the input feature and annotator clicks along the channel dimension
        combined_input = torch.cat([input_feature, annotator_clicks], dim=1)
        
        # Apply spatial attention mechanism
        cbam_feature = combined_input
        ...
        attention_map = self.sigmoid(cbam_feature)
        
        # Multiply the input feature by the attention map
        output_feature = input_feature * attention_map
...
class MixAttentionUNetClicksV2(nn.Module):
    ...
    def forward(self, x):
        ...
        x4 = self.attention(x4, c4)
        d5 = torch.cat((x4, d5), dim=1)
        d5 = self.upconv5(d5)
        
        d4 = self.up4(d5)
        x3 = self.att4(g=d4, x=x3)
        x3 = self.attention(x3, c3)
        d4 = torch.cat((x3, d4), dim=1)
        d4 = self.upconv4(d4)
        
        d3 = self.up3(d4)
        x2 = self.att3(g=d3, x=x2)
        x2 = self.attention(x2, c2)
        d3 = torch.cat((x2, d3), dim=1)
        d3 = self.upconv3(d3)
        
        d2 = self.up2(d3)
        x1 = self.att2(g=d2, x=x1)
        x1 = self.attention(x1, clicks)
