...
def validate(val_loader, model, criterion, smooth):
    ...
    with torch.no_grad():
        for input, target in val_loader:
            ...
            prec = precision(output, target)
            rec = recall(output, target)
            if save_wandb and np.random.rand() < prob:
                wandb.log({
                        'inp-tar-out': [
                            wandb.Image(input[0].cpu().numpy(), caption="Input"),
                            wandb.Image(target[0].cpu().numpy(), caption="Target"),
                            wandb.Image(output[0].cpu().numpy(), caption="Output"),
                        ]
                    })
                save_wandb = False
...
def main_hyper():
    ...
    for batch_size, model_test, augmentation, learning_rate, loss_smooth, crop_size, metas in hyperparameter_combinations:               
        ...
        if os.name == 'nt':
            ...
            if metas == 'meta_lndb':
                IMAGE_DIR = 'F:/data/LNDb/image/'
                MASK_DIR = 'F:/data/LNDb/masks/'
            elif metas == 'meta_dec':
                IMAGE_DIR = 'F:/data/MedicalDec/image/'
                MASK_DIR = 'F:/data/MedicalDec/masks/'
            else:    
