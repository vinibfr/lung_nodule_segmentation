...
def return_model(config):
    ...
    if config['name']=='NestedUNET':
    ...
    elif config['name']=='DSUNET':
        model = VITUNet(n_channels=1, n_classes=1, bilinear=True)
        ...
        model = AttentionUNet(n_classes=1, in_channel=1, out_channel=1)
    elif config['name']=='ASPPUNet': # bottleneck models
        model = ASPPUnet(1,1)
    elif config['name']=='PYBUNet':
        model = PYBUNet(1,1, config['batch_size'])
        ...
        model = SAMNestedUnet(num_classes=1)
    elif config['name']=='MixAttentionUNet':
        model = MixAttentionUNet(n_classes=1, in_channel=1, out_channel=1)
    elif config['name']=='R2AttentionUNet':
        model = R2AttentionUNet()
    elif config['name']=='R2AttentionUnetSAM':
        model = R2AttentionUNetSAM()
    elif config['name']=='SKSAMUNet':
        model = SKSAMUNet(1,1)
    elif config['name']=='SKSAMUNetV2':
        model = SKSAMUNetV2(1,1)        
    elif config['name']=='SAMAttentionUNet':
        model = SAMAttentionUNet(n_classes=1, in_channel=1, out_channel=1)        
    else:
        model = UNet(1,1) # basic model unet
    return model, config
...
def train(train_loader, model, criterion, optimizer):
    ...
    for input, target in train_loader:
        ...
        if save_wandb and np.random.rand() < prob and saves > 0:
            wandb.log({
                    'train_inp-tar-out': [
                        wandb.Image(input[0].detach().cpu().numpy(), caption="Input {}".format(str(input.shape))),
                        wandb.Image(target[0].detach().cpu().numpy(), caption="Target {}".format(str(target.shape))),
                        wandb.Image(output[0].detach().cpu().numpy(), caption="Output {}".format(str(output.shape))),
                    ]
                })
            saves -= 1
            save_wandb = False
...
def main_hyper():
    ...
    if os.name == 'nt':
        ...
        hyper_batch_size = [32]
    hyper_crop_size = [512]
    hyper_meta = ['meta_lndb','meta_dec','meta_all','meta']
        # Generate all combinations of hyperparameters
    hyperparameter_combinations = itertools.product(
        hyper_batch_size, hyper_model, hyper_augmentation, hyper_lr, hyper_crop_size, hyper_meta
    )
    runid = 1
    for batch_size, model_test, augmentation, learning_rate, crop_size, metas in hyperparameter_combinations:               
        config['augmentation'] = augmentation
        config['lr'] = learning_rate
        config['batch_size'] = batch_size
        config['name'] = model_test
        config['channels'] = 1
        file_name = config['name'] + '_final_Aug_True_batch_size_{}_epochs_{}_crop_{}_meta_{}'.format(batch_size, config['epochs'], str(crop_size), metas)
        os.makedirs('models_output_{}/{}'.format(str(crop_size),file_name),exist_ok=True)
        ...
        #save configuration
        with open('models_output_{}/{}/config.yml'.format(str(crop_size),file_name), 'w') as f:
            yaml.dump(config, f)
        ...
        if os.name == 'nt':
            ...
            IMAGE_DIR = 'D:/data/six/ImageCL' + str(crop_size)
            MASK_DIR = 'D:/data/six/MaskCL' + str(crop_size)
        else:
            print("Running on Linux")
            if metas == 'meta_lndb':
                IMAGE_DIR = '/nas-ctm01/datasets/private/LUCAS/LNdb/data/image/'
                MASK_DIR = '/nas-ctm01/datasets/private/LUCAS/LNdb/data/masks/'
            elif metas == 'meta_dec':
                IMAGE_DIR = '/nas-ctm01/datasets/private/LUCAS/DEC/data/image/'
                MASK_DIR = '/nas-ctm01/datasets/private/LUCAS/DEC/data/masks/'
            else:
        ...
        for epoch in range(config['epochs']):
            ...
            log.to_csv('models_output_{}/{}/log.csv'.format(str(crop_size),file_name), index=False)
            trigger += 1
            wandb.log({"val_dice": val_log['dice'], "val_iou": val_log['iou'], "val_loss": val_log['loss'],"train_dice": train_log['dice'], "train_iou": train_log['iou'], "train_loss": train_log['loss']})
            if val_log['dice'] > best_dice:
                torch.save(model.state_dict(), 'models_output_{}/{}/model.pth'.format(str(crop_size),file_name))
