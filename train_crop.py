...
def return_model(config):
    ...
    if config['name']=='NestedUNET':
        ...
        model = VITUNet(n_channels=1, n_classes=1, patch_size=config['patch_size'], bilinear=True)
...
def main_hyper():
    ...
    hyper_model = ['UNet','NestedUNET','AttentionUNet','SAMUNet', 'MixAttentionUNet','ASPPUNet','PYBUNet', 'SEBUNet', 'SAMUNet2','VITUNet','SAMNestedUnet','R2AttentionUNet','R2AttentionUnetSAM','SKSAMUNet','SKSAMUNetV2']
    hyper_model = ['ASPPUNet']
    ...
    for batch_size, model_test, augmentation, learning_rate, loss_smooth, crop_size, metas in hyperparameter_combinations:               
        ...
        file_name = config['name'] + 'APAGARfull_Aug_True_batch_size_{}_epochs_{}_crop_{}_{}'.format(batch_size, config['epochs'], str(crop_size),metas)
